...
[TestClass]
[DoNotParallelize]
public sealed class ReadBitsTest : IDisposable {
    ...
    [TestMethod]
    public async Task 离散输入读取测试_DiscreteInputs() {
        int length = Math.Min(_bools.Length, 200);
        TestContext.WriteLine($"Title--验证 DiscreteInputs bit[] 读取测试，目标地址：{_writeDataAdr}，数组长度：{length}");

        TestContext.WriteLine("NO.1--读取 DiscreteInputs bit[]");
        bool[] result = await _protocol.ReadBitsAsync(
            area: ModbusDataArea.DiscreteInputs,
            startAddress: _writeDataAdr,
            count: (ushort)length,
            cts: TestContext.CancellationTokenSource.Token
        ).ConfigureAwait(false);

        TestContext.WriteLine($"读取完成：长度={result.Length}");
